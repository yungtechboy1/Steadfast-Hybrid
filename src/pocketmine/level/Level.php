	...
	public function addSound(Sound $sound, array $players = \null){
		...
		if($pk !== \null){
			if(!\is_array($pk)){
				...
				foreach($players as $player) {
					if($player->is_beta) {
						foreach($pk as $key => $packet) {
							if($packet instanceof DataPacket) {
								$player->dataPacket($packet->setChannel(Network::CHANNEL_WORLD_EVENTS));
							}
						}
					}
				}
				...
				if(ADVANCED_CACHE == \true and ($cache = Cache::get("world:" . $this->getId() . ":" . $index)) !== \false){
					...
					foreach($players as $player){
						if($player->isConnected() and isset($player->usedChunks[$index])){
							$p = explode("biomestart", $cache);
							$d = explode("biomeend", $p[1]);
							$replace_biome = "biomestart".$d[0]."biomeend";

							$p = explode("packstart", $cache);
							$d = explode("packend", $p[1]);
							$replace_pack = "packstart".$d[0]."packend";

							$p = explode("heightmapstart", $cache);
							$d = explode("heightmapend", $p[1]);
							$replace_heightmap = "heightmapstart".$d[0]."heightmapend";

							if($player->is_beta) {
								if($player->is_legacy) {
									$player->sendChunk($x, $z, str_replace(array("biomestart", "biomeend", $replace_pack, $replace_heightmap), array(), $cache));
								} else {
									$player->sendChunk($x, $z, str_replace(array($replace_biome, $replace_pack,"heightmapstart", "heightmapend"), array(""), $cache));
								}
							} else {
								$player->sendChunk($x, $z, str_replace(array("biomestart", "biomeend", "packstart", "packend", $replace_heightmap), array(), $cache));
							}
						}
					}
	...
	public function chunkRequestCallback($x, $z, $payload){
		...
		if(isset($this->chunkSendTasks[$index])){
			...
			foreach($this->chunkSendQueue[$index] as $player){
				/** @var Player $player */
				if($player->isConnected() and isset($player->usedChunks[$index])){
					$p = explode("biomestart", $payload);
					$d = explode("biomeend", $p[1]);
					$replace_biome = "biomestart".$d[0]."biomeend";

					$p = explode("packstart", $payload);
					$d = explode("packend", $p[1]);
					$replace_pack = "packstart".$d[0]."packend";

					$p = explode("heightmapstart", $payload);
					$d = explode("heightmapend", $p[1]);
					$replace_heightmap = "heightmapstart".$d[0]."heightmapend";

					if($player->is_beta) {
						if($player->is_legacy) {
							$player->sendChunk($x, $z, str_replace(array("biomestart", "biomeend", $replace_pack, $replace_heightmap), array(), $payload));
						} else {
							$player->sendChunk($x, $z, str_replace(array($replace_biome, $replace_pack,"heightmapstart", "heightmapend"), array(""), $payload));
						}
					} else {
						$player->sendChunk($x, $z, str_replace(array("biomestart", "biomeend", "packstart", "packend", $replace_heightmap), array(), $payload));
					}
				}
			}
			unset($this->chunkSendQueue[$index]);
