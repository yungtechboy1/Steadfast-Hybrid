...
use pocketmine\network\protocol\OldPlayerArmorEquipmentPacket;
use pocketmine\network\protocol\OldPlayerEquipmentPacket;
...
class PlayerInventory extends BaseInventory{
	...
	public function setHeldItemIndex($index){
		if($index >= 0 and $index < $this->getHotbarSize()){
			...
			$viewers = $this->getHolder()->getViewers();
			foreach($viewers as $player) {
				if($player instanceof Player) {
					if($player->is_beta) {
						$pk = new PlayerEquipmentPacket();
					} else {
						$pk = new OldPlayerEquipmentPacket();
					}
					...
					Server::broadcastPacket([$player], $pk->setChannel(Network::CHANNEL_ENTITY_SPAWNING));
				}
			}
		}
	}
	...
	public function sendArmorContents($target){
		...
		if($target[0]->is_beta) {
			...
			$pk = new OldPlayerArmorEquipmentPacket();
		}
		$pk->eid = $this->getHolder()->getId();
		$pk->slots = $slots;
		$pk->encode();
