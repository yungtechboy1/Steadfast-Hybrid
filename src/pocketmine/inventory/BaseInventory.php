...
use pocketmine\network\protocol\OldContainerSetSlotPacket;
			...
			foreach($target as $t) {
				if($t instanceof Player) {
					if($t->is_beta) {
						...
						$pk = new OldContainerSetSlotPacket();
					}
					$pk->slot = $index;
					$pk->item = clone $this->getItem($index);

					if(($id = $t->getWindowId($this)) === -1){
						$this->close($t);
						continue;
					}
					$pk->windowid = $id;
					$t->dataPacket($pk->setChannel(Network::CHANNEL_WORLD_EVENTS));
				}
			}
		} else {
			$t = $target;
